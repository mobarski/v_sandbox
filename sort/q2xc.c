...
static int main__compare_int(int a, int b);
static void main__quicksort_int(array_int* tab, u64 left, u64 right);
...
Option_array_byte os__read_bytes(string path) {
	...
	fclose(fp);
	Option_array_byte _t73;
	opt_ok2(&(array_byte[]) { array_slice(res, 0, nr_read_elements * fsize) }, (OptionBase*)(&_t73), sizeof(array_byte));
	return _t73;
}
...
Option_string os__read_file(string path) {
	...
	if (isnil(fp)) {
		Option _t74 = v_error(_STR("failed to open file \"%.*s\000\"", 2, path));
		return *(Option_string*)&_t74;
	}
	fseek(fp, 0, SEEK_END);
	int fsize = ftell(fp);
	rewind(fp);
	{ // Unsafe block
		...
		// Defer end
		Option_string _t75;
		opt_ok2(&(string[]) { byteptr_vstring_with_len(str, fsize) }, (OptionBase*)(&_t75), sizeof(string));
		return _t75;
	}
	// Defer begin
	fclose(fp);
...
// $if  windows {
	...
	if (CopyFile(string_to_wide(w_old), string_to_wide(w_new), false) == 0) {
		...
		Option _t77 = error_with_code(_STR("failed to copy %.*s\000 to %.*s", 2, old, v_new), ((int)(result)));
...
// $if  windows {
	...
	return _pclose(f);
#else
	multi_return_int_bool mr_8323 = os__posix_wait4_to_exit_status(pclose(f));
	int ret = mr_8323.arg0;
	return ret;
#endif
// } windows
}
...
// $if  windows {
	...
	if (rc == -1) {
		Option _t120 = v_error(_STR("Failed to remove \"%.*s\000\"", 2, path));
...
// $if  windows {
	...
	if (rc == 0) {
		Option _t123 = v_error(_STR("Failed to remove \"%.*s\000\"", 2, path));
...
Option_os__File os__open(string path) {
	...
	opt_ok2(&(os__File[]) { (os__File){.cfile = cfile,.fd = fd,.is_opened = true,} }, (OptionBase*)(&_t170), sizeof(os__File));
	return _t170;
}
...
Option_os__File os__create(string path) {
	...
	opt_ok2(&(os__File[]) { (os__File){.cfile = cfile,.fd = fd,.is_opened = true,} }, (OptionBase*)(&_t172), sizeof(os__File));
	return _t172;
}
...
Option_array_string os__ls(string path) {
	...
	if (!os__is_dir(path)) {
		Option _t174 = v_error(_STR("ls() couldnt open dir \"%.*s\000\": directory does not exist", 2, path));
	...
	Option_array_string _t177;
	opt_ok2(&(array_string[]) { dir_files }, (OptionBase*)(&_t177), sizeof(array_string));
	return _t177;
}
...
Option_bool os__mkdir(string path) {
	...
	if (!CreateDirectory(string_to_wide(apath), 0)) {
		Option _t179 = v_error(string_add(_STR("mkdir failed for \"%.*s\000\", because CreateDirectory returned ", 2, apath), os__get_error_msg(((int)(GetLastError())))));
		return *(Option_bool*)&_t179;
	}
	Option_bool _t180;
	opt_ok2(&(bool[]) { true }, (OptionBase*)(&_t180), sizeof(bool));
	return _t180;
}
	...
	{ // Unsafe block
		...
		while (1) {
			...
			if (status == _const_os__success) {
				Option_string _t182;
				opt_ok2(&(string[]) { string_from_wide2(buf, sz) }, (OptionBase*)(&_t182), sizeof(string));
				return _t182;
			} else {
				Option _t183 = v_error(tos_lit("Cannot get file name from handle"));
				return *(Option_string*)&_t183;
			};
		}
	}
...
Option_os__Result os__exec(string cmd) {
	if (string_contains(cmd, tos_lit(";")) || string_contains(cmd, tos_lit("&&")) || string_contains(cmd, tos_lit("||")) || string_contains(cmd, tos_lit("\n"))) {
		Option _t185 = v_error(tos_lit(";, &&, || and \\n are not allowed in shell commands"));
		return *(Option_os__Result*)&_t185;
	}
	...
	if (!create_pipe_ok) {
		...
		Option _t186 = error_with_code(_STR("exec failed (CreatePipe): %.*s", 1, error_msg), error_num);
		return *(Option_os__Result*)&_t186;
	}
	bool set_handle_info_ok = SetHandleInformation(child_stdout_read, HANDLE_FLAG_INHERIT, 0);
	if (!set_handle_info_ok) {
		int error_num = ((int)(GetLastError()));
		string error_msg = os__get_error_msg(error_num);
		Option _t187 = error_with_code(_STR("exec failed (SetHandleInformation): %.*s", 1, error_msg), error_num);
		return *(Option_os__Result*)&_t187;
	}
	...
	if (!create_process_ok) {
		int error_num = ((int)(GetLastError()));
		string error_msg = os__get_error_msg(error_num);
		Option _t188 = error_with_code(_STR("exec failed (CreateProcess) with code %"PRId32"\000: %.*s\000 cmd: %.*s", 3, error_num, error_msg, cmd), error_num);
		return *(Option_os__Result*)&_t188;
	}
	...
	Option_os__Result _t189;
	opt_ok2(&(os__Result[]) { (os__Result){.exit_code = ((int)(exit_code)),.output = soutput,} }, (OptionBase*)(&_t189), sizeof(os__Result));
	return _t189;
}
...
Option_bool os__symlink(string origin, string target) {
	...
	Option _t191 = v_error(os__get_error_msg(((int)(GetLastError()))));
	return *(Option_bool*)&_t191;
}
...
Option_bool os__is_writable_folder(string folder) {
	if (!os__exists(folder)) {
		Option _t192 = v_error(_STR("`%.*s\000` does not exist", 2, folder));
		return *(Option_bool*)&_t192;
	}
	if (!os__is_dir(folder)) {
		Option _t193 = v_error(tos_lit("`folder` is not a folder"));
		return *(Option_bool*)&_t193;
	}
	...
	Option_os__File _t194 = os__open_file(tmp_perm_check, tos_lit("w+"), (varg_int){.len=1,.args={0700}});
	if (!_t194.ok) {
		string err = _t194.v_error;
		int errcode = _t194.ecode;
		Option _t195 = v_error(_STR("cannot write to folder %.*s\000: %.*s", 2, folder, err));
		return *(Option_bool*)&_t195;
	}
	os__File f = *(os__File*)_t194.data;
	os__File_close(&f);
...
Option_time__Time time__parse(string s) {
	Option_int _t200 = string_index(s, tos_lit(" "));
	if (!_t200.ok) {
		string err = _t200.v_error;
		int errcode = _t200.ecode;
		Option _t201 = v_error(_STR("Invalid time format: %.*s", 1, s));
		return *(Option_time__Time*)&_t201;
	}
	int pos = *(int*)_t200.data;
	string symd = string_substr(s, 0, pos);
	array_string ymd = string_split(symd, tos_lit("-"));
	if (ymd.len != 3) {
		Option _t202 = v_error(_STR("Invalid time format: %.*s", 1, s));
		return *(Option_time__Time*)&_t202;
	}
	...
	Option_time__Time _t203;
	opt_ok2(&(time__Time[]) { res }, (OptionBase*)(&_t203), sizeof(time__Time));
	return _t203;
}
...
Option_time__Time time__parse_rfc2822(string s) {
	...
	if (fields.len < 5) {
		Option _t204 = v_error(_STR("Invalid time format: %.*s", 1, s));
		return *(Option_time__Time*)&_t204;
	}
	Option_int _t205 = string_index(_const_time__months_string, (*(string*)array_get(fields, 2)));
	if (!_t205.ok) {
		string err = _t205.v_error;
		int errcode = _t205.ecode;
		Option _t206 = v_error(_STR("Invalid time format: %.*s", 1, s));
		return *(Option_time__Time*)&_t206;
	}
	int pos = *(int*)_t205.data;
	int mm = pos / 3 + 1;
...
Option_time__Time time__parse_iso8601(string s) {
	...
	if (count != 11) {
		Option _t207 = v_error(tos_lit("Invalid 8601 format"));
		return *(Option_time__Time*)&_t207;
	}
	if (time_char != 'T' && time_char != ' ') {
		Option _t208 = v_error(tos_lit("Invalid 8601 format, expected space or `T` as time separator"));
		return *(Option_time__Time*)&_t208;
	}
	if (plus_min != '+' && plus_min != '-') {
		Option _t209 = v_error(tos_lit("Invalid 8601 format, expected `+` or `-` as time separator"));
		return *(Option_time__Time*)&_t209;
	}
	...
	Option_time__Time _t210;
	opt_ok2(&(time__Time[]) { time__to_local_time(t) }, (OptionBase*)(&_t210), sizeof(time__Time));
	return _t210;
}
...
Option_int time__days_in_month(int month, int year) {
	if (month > 12 || month < 1) {
		Option _t211 = v_error(_STR("Invalid month: %"PRId32"", 1, month));
		return *(Option_int*)&_t211;
	}
	int extra = (month == 2 && time__is_leap_year(year) ? (1) : (0));
	int res = (*(int*)array_get(_const_time__month_days, month - 1)) + extra;
	Option_int _t212;
	opt_ok2(&(int[]) { res }, (OptionBase*)(&_t212), sizeof(int));
	return _t212;
}
...
static inline int main__compare_int(int a, int b) {
	if (a < b) {
		return -1;
	}
	if (a > b) {
...
static void main__quicksort_int(array_int* tab, u64 left, u64 right) {
	...
	if (left < j) {
		main__quicksort_int(tab, left, j);
	}
	if (right > i) {
		main__quicksort_int(tab, i, right);
	}
}
...
static void main__main() {
	...
	for (int _t215 = 0; _t215 < 100; ++_t215) {
		...
		if (int_arr_eq(a, b)) {} else {
			...
			v_assert_meta_info__t218.fpath = tos_lit("quick2gencmp.v");
			v_assert_meta_info__t218.line_nr = 44;
			v_assert_meta_info__t218.fn_name = tos_lit("main.main");
			v_assert_meta_info__t218.src = tos_lit("a == b");
			v_assert_meta_info__t218.op = tos_lit("==");
			v_assert_meta_info__t218.llabel = tos_lit("a");
			v_assert_meta_info__t218.rlabel = tos_lit("b");
			v_assert_meta_info__t218.lvalue = array_int_str(  a) /* typeof: v.ast.Ident type: 32 */ ;
			v_assert_meta_info__t218.rvalue = array_int_str(  b) /* typeof: v.ast.Ident type: 32 */ ;
			__print_assert_failure(&v_assert_meta_info__t218);
			v_panic(tos_lit("Assertion failed..."));
			exit(1);
		}
	}
	println(i64_str(total1));
	println(i64_str(total2));
	println(_STR("quicksort<T>: %8"PRId64"\000 µs", 2, total1));
	println(_STR("  array.sort: %8"PRId64"\000 µs", 2, total2));
	println(_STR("quicksort<T>/array.sort: %g", 1, ((f64)(total1)) / ((f64)(total2))));
}
